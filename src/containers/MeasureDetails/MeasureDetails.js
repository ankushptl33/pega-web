...
import { Grid, Select, Typography, Card, MenuItem } from '@material-ui/core';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { Link } from 'react-router-dom';
import MasterTab from '../../components/MasterTab';
import ComponentStacker from '../../components/ComponentStacker';
import BasicTable from '../../components/BasicTable/BasicTable';
import { getPerformanceTrendInfoData } from '../../redux/actions/performanceTrendInfoAction';
import MeasurePerformanceStats from '../../components/MeasurePerformanceStats/MeasurePerformanceStats';
import { updateMeasureIDSelection } from '../../redux/actions/practiceMeasureAction';
...
class MeasureDetails extends Component {
  ...
  onMeasureChangeHandler = event => {
    ...
    if (this.props.selectedMeasureId != event.target.value) {
      ...
      selectedMeasureData[0].performance.performanceData.MeasureId =
        event.target.value;
      ...
      this.props.loadMeasurePerformance(
        selectedMeasureData[0].performance.performanceData,
      );
    }
  };
  ...
  getmeasurePerformanceObj = () => {
    ...
    return [
      {
        label: this.props.MeasurePerformanceData.performanceData
          .performanceText,
        data: this.props.MeasurePerformanceData.performanceData.performance,
      },
      {
        label: this.props.MeasurePerformanceData.performanceData.benchMark[2]
          .label,
        data: this.props.MeasurePerformanceData.performanceData.benchMark[2]
          .data,
      },
      {
        label: this.props.MeasurePerformanceData.performanceData.benchMark[0]
          .label,
        data: this.props.MeasurePerformanceData.performanceData.benchMark[0]
          .data,
      },
    ];
  };
  ...
  render() {
    if (this.props.IsMeasurePerformanceLoading) {
      ...
      masterTabProp[1].menuItem = `CLINICIANS (${
        this.props.SelectedPractice.providerCnt
      })`;
      masterTabProp[2].menuItem = `LOCATIONS (${
        this.props.SelectedPractice.locationCnt
      })`;
      ...
      return (
        <Grid container className="measure-details--page__wrapper">
          ...
          <Grid item xs={12}>
            <Grid
              container
              className="practice-measure-performance__tab-container">
              <Grid item xs={12}>
