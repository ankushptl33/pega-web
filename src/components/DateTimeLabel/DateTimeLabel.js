    ...
    <List className="datatime-flexContainer">
      {
        props.dates.map((obj, i) => {
          ...
          date = date.split(" ");
          let dayString = []
          dayString = date.map((singledate,i) => (singledate.match(/[0-9]+/g) && singledate.match(/[a-zA-Z]+/g)) ? (React.createElement('strong', {key:i}, " " + singledate.match(/[0-9]+/g).toString(), React.createElement('sup', {key:i}, singledate.match(/[a-z]+/g).toString()))) : " " + singledate)
      
          return <ListItem key={i} className="datatime-content">
            <Tooltip title={obj.title ? obj.title : ''}>
              <span className="datatime-label">{obj.label} :</span>
            </Tooltip>
            <span className="datatime-numbers__span">
              { obj.date ?   React.createElement('strong', {key:i}, dayString) : <div>no data available.</div>
              }
            </span>
          </ListItem>
        })}
