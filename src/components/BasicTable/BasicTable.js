    ...
    columnValues = cols.map((colData, i) => {
      return colData.Description != undefined ? (
        <Tooltip title={colData.Description}>
          <TableCell className="basicTable__table-cell" key={i}>{colData.label}</TableCell>
        </Tooltip>
      ) : (
        <TableCell className="basicTable__table-cell" key={i}>{colData.label}</TableCell>
      );
    ...
    if (cols != null || cols != undefined) {
      dataValues = data.map((item, index) => {
        ...
        cells = cols.map((colData, i) => {
          ...
          dataColumns.push(React.createElement(TableCell, {
            "className" : "basicTable__cell",key:i
        }, null, ChildComp));
          //return <TableCell key={i}>{item[colData.key]}</TableCell>;
        });
        return <TableRow className="basicTable__row" key={index}>{dataColumns}</TableRow>;
      });
    } else {
      dataValues = data.map(row, key => (
        <TableRow key={key} className="basicTable__row">
          {Object.values(row).map(rowValue => (
            <TableCell className="basicTable__table-cell" key={key}>{rowValue}</TableCell>
          ))}
        </TableRow>
      ));
    }
