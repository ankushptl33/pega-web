  ...
  props.data.map((data, dataIndex) => {
    props.columnDefs.map((column, index) => {
      ...
      if (headerColumns.length != props.columnDefs.length) {
        ...
        headerColumns.push(
          React.createElement(Grid, columnStyle, headerColumn),
        );
      }
...
function createStyleObject(columnDef, key) {
  try {
    ...
    columnStyle['className'] = `tableColumn ${customColumnClass} ${[
      ...cssClasses,
    ]}`;
  ...
  try {
    ...
    if (component !== null && component !== undefined) {
      ...
      if (childComponent !== null && childComponent !== undefined) {
        ...
        let dataColumn = null;
        dataColumn = CreateDyanamicComponent(childComponent, propsString, [
          ...childrens,
        ]);
