...
const MeasurePerformanceStats = props => {
  ...
  let arrow, colorArrow;
  let leftFloatedMeasures = null;
  let rightFloatedMeasures = null;

  if (isinverse) {
    arrow = configuration.iconArrowDown
      ? configuration.iconArrowDown
      : ['fal', 'arrow-up'];
  } else {
    arrow = configuration.iconArrowUp
      ? configuration.iconArrowUp
      : ['fal', 'arrow-down'];
  }

  if (isinverse) {
    colorArrow = configuration.colorArrowDown
      ? configuration.colorArrowDown
      : 'green';
  } else {
    colorArrow = configuration.colorArrowUp
      ? configuration.colorArrowUp
      : 'green';
  }
